...
public class BrokerApiRestClientImpl implements BrokerApiRestClient {
    ...
    public List<Trade> getMyTrades(MyTradeRequest request) {
        return BrokerApiServiceGenerator.executeSync(brokerApiService.getMyTrades(
                request.getFromId(), request.getToId(), request.getStartTime(), request.getEndTime(),
                request.getLimit(),
                request.getRecvWindow(), request.getTimestamp(), request.getSymbol())
        );
    }
    ...
    @Override
    public Object getBalanceFlow(String startTime, String endTime, Long limit, Long offset, Integer orgId) {
        return BrokerApiServiceGenerator.executeSync(brokerApiService.getBalanceFlow(startTime, endTime, limit, offset, orgId));
    }
}
