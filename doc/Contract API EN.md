...
`state`|string|`NO`|`current`|Get `current` or `past` funding rate. **Note: Currently only `current` is supported.**
...
### **Request Url:**
```bash
GET /openapi/contract/v1/historyFundingRate
```

### **Parameters:**
name|type|required|default|description
------------ | ------------ | ------------ | ------------ | ----
`symbol`|string|`NO`||The contract to be returned. If not sent, records for all symbols will be returned.
`fromId`|long|`NO`|`0`|Pagination parameter. Returns records with id < fromId. **Results are always sorted by id in descending order.**
`endId`|long|`NO`|`0`|Pagination parameter. Returns records with id > endId. **Results are always sorted by id in descending order.**
`limit`|integer|`NO`|`20`|Number of entries returned.

**Pagination Logic:**
- Only `fromId`: Returns records with id < fromId, sorted by id DESC (newer to older)
- Only `endId`: Returns records with id > endId, sorted by id DESC (newer to older)
- Both `fromId` and `endId`: Returns records with endId < id < fromId, sorted by id DESC
- Neither: Returns the latest `limit` records, sorted by id DESC

### **Response:**
name|type|example|description
------------ | ------------ | ------------ | ------------
`id`|long|`1234567890`|Record ID.
`symbol`|string|`ETH-SWAP-USDT`|Name of the contract.
`settleTime`|long|`1570708800000`|Funding rate settlement timestamp (ms).
`settleRate`|string|`0.0001`|Funding rate at settlement.
...
Retrieve detailed trade history with enhanced information. This API endpoint requires your request to be signed.
...
### **Request Url:**
```bash
GET /openapi/contract/v1/myTradeDetails
```
...
`limit`|integer|`NO`|`20`|Number of entries returned (max 500).

### **Response:**
Name|type|example|description
------------ | ------------ | ------------ | ------------
`tradeId`|long|`469968263995080704`|The ID for the trade.
`accountId`|long|`456552319339779840`|Account ID.
`orderId`|long|`469968263793737728`|Order ID.
`matchOrderId`|long|`436002617267469062`|Matched order ID.
`symbolId`|string|`BTC-PERP-REV`|Name of the contract.
`feeTokenId`|string|`TBTC`|Fee token name.
`side`|string|`BUY_OPEN`|Direction of the order. Possible values include `BUY_OPEN`, `SELL_OPEN`, `BUY_CLOSE`, and `SELL_CLOSE`.
`quantity`|string|`100`|Trade quantity.
`amount`|string|`0.01172`|Trade amount.
`price`|string|`8531.17`|Trade price.
`tokenFee`|string|`0.00000586`|Fee amount.
`isMarker`|boolean|`true`|Whether this trade is maker.
`updatedAt`|long|`1570760582848`|Update timestamp (ms).
`isClose`|boolean|`false`|Whether this is a close position trade.
`extraInfo`|long|`0`|Extra information flag.
...
Retrieve detailed trade history with symbol filter (V2). This API endpoint requires your request to be signed.
...
### **Request Url:**
```bash
GET /openapi/contract/v1/myTradeDetailsV2
```

### **Parameters:**
Parameter|type|required|default|description
------------ | ------------ | ------------ | ------------ | -------
`symbolId`|string|`NO`||Name of the contract. If not sent, trades for all symbols will be returned.
`fromId`|long|`NO`||TradeId to fetch from.
`toId`|long|`NO`||TradeId to fetch to.
`startTime`|long|`NO`||Start timestamp (ms).
`endTime`|long|`NO`||End timestamp (ms).
...
### **Response:**
Same as `myTradeDetails` endpoint.
...
Retrieve the best bid and ask orders for a specific contract. This API endpoint requires your request to be signed.
...
### **Request Url:**
```bash
GET /openapi/contract/v1/getBestOrder
```
...
Name|type|example|description
------------ | ------------ | ------------ | ------------
`price`|string|`8531.17`|Current price.
`bid`|object|(see below)|Best bid order information.
`ask`|object|(see below)|Best ask order information.

In the `bid` and `ask` fields:

Name|type|example|description
------------ | ------------ | ------------ | ------------
`time`|long|`1570760582848`|Order timestamp (ms).
`orderId`|long|`469968263793737728`|Order ID.
`accountId`|long|`456552319339779840`|Account ID.
`price`|string|`8531.17`|Order price.
`origQty`|string|`100`|Original order quantity.
`type`|string|`LIMIT`|Order type.
`side`|string|`BUY`|Order direction.
...
```js
{
  ...
  'ask': {
    'time': 1570760582850,
    'orderId': 469968263793737729,
    'accountId': 456552319339779841,
    'price': '8532.00',
    'origQty': '50',
    'type': 'LIMIT',
    'side': 'SELL',
